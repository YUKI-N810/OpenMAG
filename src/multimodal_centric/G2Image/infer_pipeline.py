...
class InstructG2IPipeline:
    def __init__(self, sd_pipe, sd_model_dir, image_encoder_path, proj_ckpt, gnn_ckpt, num_tokens, cross_attention_freq, neighbor_num, device, args, cfg):
        self.device = device
        ...
        self.args = args
        self.cfg = cfg 
        self.pretrained_image_embs = torch.from_numpy(np.load(args.pretrained_image_embedding_dir)).to(device)
    ...
    @classmethod
    def from_pretrained(cls, sd_model_dir, neighbor_num, cfg, cache_dir='cache/', device='cpu', args=None):
        ...
        print(f"[Info] Architecture Flags -> Use QFormer: {args.use_qformer}, Use GNN: {args.use_gnn}")

        gnn_ckpt = torch.load(os.path.join(sd_model_dir, "gnn", "ckpt.pt"), map_location="cpu")
        
         # === QFormer ===
        if args.use_qformer:
            proj_ckpt_path = os.path.join(sd_model_dir, "image_proj_model", "ckpt.pt")
            ...
            if "crossattention.q_proj.weight" in gnn_ckpt: 
        ...
        # === Simple Baseline (Projector Only) ===
        else:
            proj_ckpt_path = os.path.join(sd_model_dir, "image_proj_model", "ckpt.pt")
            
            if os.path.exists(proj_ckpt_path):
                proj_ckpt = torch.load(proj_ckpt_path, map_location="cpu")
                if 'norm.weight' in proj_ckpt:
                    num_tokens = int(proj_ckpt['proj.weight'].shape[0] / proj_ckpt['norm.weight'].shape[0])
                else:
                    num_tokens = 1
                del proj_ckpt 
            ...
            print(f"[Info] Detected num_tokens for Baseline: {num_tokens}")
            cross_attention_freq = 1
        
        return cls(sd_init, sd_model_dir, os.path.join(sd_model_dir, 'image_encoder'),
                   proj_ckpt_path, os.path.join(sd_model_dir, "gnn", "ckpt.pt"),
                   num_tokens, cross_attention_freq, neighbor_num, device, args, cfg)
        
    def init_proj_and_gnn(self):
        # === QFormer ===
        if self.args.use_qformer:
            print("Initializing model: QFormer")
            image_proj_model = ImageProjModel(
                cross_attention_dim=self.pipe.unet.config.cross_attention_dim,
                clip_embeddings_dim=self.args.image_encoder_dim,
                clip_extra_context_tokens=self.num_tokens,
            ).to(self.device, dtype=torch.float16)
            
            # WARNING: Hardcoded path detected below
            encoder_config = CLIPConfig.from_pretrained(self.sd_model_dir, subfolder="text_encoder", cache_dir="/shared/data/bowenj4/hf-cache")
        ...
        # === GNN ===
            ...
            print(f"Initializing model: GNN ({model_name})")
            
            in_dim = self.args.image_encoder_dim
            hidden_dim = self.pipe.unet.config.cross_attention_dim
            num_layers = self.args.num_layers if hasattr(self.args, "num_layers") else 2 
            
            gnn = None
            if model_name == "gcn": gnn = GCN(in_dim, hidden_dim, num_layers, 0.0)
            elif model_name == "gat": gnn = GAT(in_dim, hidden_dim, num_layers, self.args.heads, 0.0)
            elif model_name == "sage": gnn = GraphSAGE(in_dim, hidden_dim, num_layers, 0.0)
            elif model_name == "mlp": gnn = MLP(in_dim, hidden_dim, num_layers, 0.0)
            elif model_name == "gin": gnn = GIN(in_dim, hidden_dim, num_layers, 0.0)
            elif model_name == "chebnet": gnn = ChebNet(in_dim, hidden_dim, num_layers, K=2, dropout=0.0)
            elif model_name == "lgmrec":
                gnn = LGMRec(in_dim=in_dim, hidden_dim=hidden_dim, num_layers=3, dropout=0.2, hyper_num=64, alpha=0.1)
            elif model_name == "dgf":
                gnn = DGF(
                    v_feat_dim=in_dim,
                    t_feat_dim=hidden_dim,
                ...
                gnn = GCN(in_dim, hidden_dim, num_layers, 0.0) # Fallback

            weight_dtype = torch.float32 if model_name == "dgf" else torch.float16
            return None, gnn.to(self.device, dtype=weight_dtype)
            
        # === Simple Baseline (Projector Only) ===
        else:
            print("Initializing model: Simple Baseline (Projection Only)")
            image_proj_model = ImageProjModel(
                cross_attention_dim=self.pipe.unet.config.cross_attention_dim,
                clip_embeddings_dim=self.args.image_encoder_dim,
                clip_extra_context_tokens=self.num_tokens, 
    ...
    def get_image_embeds(self, neighbor_image=None, clip_image_embeds=None):
        ...
        if neighbor_image is not None:
            ...
            clip_image_embeds = []
            for sample in neighbor_image:
                if sample == 'Empty': clip_image_embeds.append(torch.zeros((self.args.image_encoder_dim,), dtype=torch.float16).to(self.device))
                else: clip_image_embeds.append(self.pretrained_image_embs[int(sample)].type(torch.float16))
            clip_image_embeds = torch.stack(clip_image_embeds)
        else:
            clip_image_embeds = clip_image_embeds.to(self.device, dtype=torch.float16)
        
        if len(clip_image_embeds.shape) == 2: clip_image_embeds = clip_image_embeds.unsqueeze(0)
        if reshape_flag: clip_image_embeds = clip_image_embeds.view(bz, self.neighbor_num, -1)
    ...
    def __call__(self, neighbor_image=None, neighbor_mask=None, clip_image_embeds=None, prompt=None, negative_prompt=None, num_samples=1, seed=None, guidance_scale=7.5, num_inference_steps=50, **kwargs):
        if not isinstance(neighbor_mask, torch.LongTensor): neighbor_mask = torch.LongTensor(neighbor_mask)

        image_prompt_embeds, uncond_image_prompt_embeds = self.get_image_embeds(neighbor_image=neighbor_image, clip_image_embeds=clip_image_embeds)
        ...
        with torch.inference_mode():
            prompt_embeds_, negative_prompt_embeds_, text_input_ids, negative_text_input_ids = _encode_prompt(
                self.pipe, prompt, device=self.device, num_images_per_prompt=num_samples, do_classifier_free_guidance=True, negative_prompt=negative_prompt
            )
            
            # 1. QFormer: Has Projector and GNN
            if self.image_proj_model is not None and self.gnn is not None:
                neighbor_mask = torch.repeat_interleave(neighbor_mask.repeat(num_samples, 1), self.num_tokens, dim=1)
            ...
            # 2. GNN: No Projector but has GNN
            elif self.gnn is not None:
                # Construct fully connected graph
                bz_curr, num_nb, dim = image_prompt_embeds.shape
                base_idx = torch.arange(num_nb, device=self.device)
                edge_block = torch.stack([base_idx.repeat_interleave(num_nb), base_idx.repeat(num_nb)], dim=0)
                edge_index = torch.cat([edge_block + i * num_nb for i in range(bz_curr)], dim=1)
                
                # Flatten features
                x_flat = image_prompt_embeds.view(-1, dim)
                x_uncond_flat = uncond_image_prompt_embeds.view(-1, dim)

                # [Modified] DGF special branch: pass in text features, unpack return values
                if self.gnn.__class__.__name__ == 'DGF':
                    # 1. Process text features (Positive)
                    # prompt_embeds_ shape: [Total_Batch, Seq, Dim] (Total_Batch = B * num_samples)
                    t_feat = prompt_embeds_.mean(dim=1) # [Total_Batch, Dim]
                    t_feat = t_feat.repeat_interleave(num_nb, dim=0) # [Total_Batch * K, Dim]
                    
                    # Concatenate and forward
                    x_in = torch.cat([x_flat, t_feat], dim=-1)
                    gnn_out = self.gnn(x_in, edge_index)[0] # Get the 0th return value
                    
                    # 2. Process text features (Negative/Uncond)
                    t_feat_uncond = negative_prompt_embeds_.mean(dim=1)
                    t_feat_uncond = t_feat_uncond.repeat_interleave(num_nb, dim=0)
                    
                    # Concatenate and forward
                    x_in_uncond = torch.cat([x_uncond_flat, t_feat_uncond], dim=-1)
                    uncond_out = self.gnn(x_in_uncond, edge_index)[0]
                else:
                    # Normal GNN Forward
                    gnn_out = self.gnn(x_flat, edge_index)
                    uncond_out = self.gnn(x_uncond_flat, edge_index)
                    
                    # Compatible with cases where normal GNN might return a tuple
                    if isinstance(gnn_out, tuple): gnn_out = gnn_out[0]
                    if isinstance(uncond_out, tuple): uncond_out = uncond_out[0]
                
                gnn_prompt_embeds = gnn_out.view(bz_curr, num_nb, -1)
                uncond_gnn_prompt_embeds = uncond_out.view(bz_curr, num_nb, -1)
            
            # 3. Baseline: Has Projector but no GNN
            else:
                # Pure projection concatenation
                gnn_prompt_embeds = image_prompt_embeds
                uncond_gnn_prompt_embeds = uncond_image_prompt_embeds
        ...
        return self.pipe(
            ...
            generator=get_generator(seed, self.device), **kwargs
        )
